...
int_base_sample <- readRDS("data/pulled/cstat_us_sample.RDS") %>%
  filter(
    indfmt == "INDL",
    fic == "USA" |  acctstd %in% c("DI", "DU", "US"),
    !is.na(at),
    at > 0,
    sale > 0
  ) %>%
...
mj_int <- estimate_mj_accruals(int_base_sample)
dd_int <- estimate_dd_accruals(int_base_sample)
...
smp_int<- expand_grid(
  gvkey = unique(int_base_sample$gvkey),
  fyear = unique(int_base_sample$fyear)
) %>%
  arrange(gvkey, fyear) %>%
  left_join(int_base_sample, by = c("gvkey", "fyear")) %>%
  left_join(mj_int, by = c("gvkey", "fyear")) %>%
  left_join(dd_int, by = c("gvkey", "fyear")) %>%
  mutate(
    ta = at,
    avgta = (at + lag(at))/2,
    sales = sale,
  ...
  select(
    ...
    ppe_ta, int_ta, gwill_ta, ceq_ta, leverage, 
    acq_sales, cogs_sales, ebit_sales, 
    ebit_avgta, cfo_avgta, tacc_avgta, fic, acctstd
  ) %>% filter(!is.na(mj_da))

saveRDS(smp, "data/generated/acc_sample.rds")
saveRDS(smp_int, "data/generated/acc_sample_int.rds")
